...
folder    <- "./../../Projeto"                  # data files folder
baseName  <- "Cenario_"                     # data files base name (same as .lsd file)
nExp      <- 7                          # number of experiments (sensitivity/different cases)
iniDrop   <- 200                          # initial time steps to drop from analysis (0=none)
nKeep     <- 200                       # number of time steps to keep (-1=all)
...
expVal <- c( "Cenário 0", 
             ...
             "Cenário 5",
             "Cenário 6",
             "Cenário 7")                           # case parameter values

# Aggregated variables to use

aggrVars <- c(  "GDP_G",        # Real GDP Growth
                        "P_G"          # Price Growth, Inflation - CPI?
                        )
...
bCase     <- 0      # experiment to be used as base case
CI        <- 0.95   # desired confidence interval
warmUpPlot<- 200    # number of "warm-up" runs for plots
nTplot    <- 200     # last period to consider for plots (-1=all)
warmUpStat<- 200    # warm-up runs to evaluate all statistics
nTstat    <- 200     # last period to consider for statistics (-1=all)
